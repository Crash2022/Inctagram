...
export const CustomSelect = ({options, onChange, value}: Props) => {
  ...
  return (
      <Select
        ...
        classNames={{
          control: state =>
            clsx(cls.select_control, state.isFocused && cls.select_focus),
          singleValue: () => cls.currentValue,
          indicatorSeparator: () => cls.separator,
          placeholder: () => cls.placeholder,
          menu: () => cls.menu,
          menuList: () => cls.menu_list,
          option: state => clsx(cls.option, state.isFocused && cls.option_active, state.isSelected && cls.option_selected),
